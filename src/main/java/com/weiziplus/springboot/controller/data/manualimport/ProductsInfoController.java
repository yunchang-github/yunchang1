...
import com.weiziplus.springboot.service.data.manualimport.ProductsInfoService;
...
@Api(value="ProductsInfoController",tags={"商品信息表"})
@RestController
//@ApiIgnore
@AdminAuthToken
@RequestMapping("/pc/data/manualImport/productsInfo")
public class ProductsInfoController {
    ...
    @PostMapping("/add")
    public ResultUtil add(@ApiParam(value = "商品信息表", required = true)ProductsInfo productsInfo) {
        return service.add(productsInfo);
    }
    ...
    @PostMapping("/update")
    public ResultUtil update(@ApiParam(value = "商品信息表", required = true)ProductsInfo productsInfo) {
        return service.update(productsInfo);
    }
