...
import com.weiziplus.springboot.service.data.manualimport.MergeAsinService;
 ...
 * @data 2019/7/19 8:38
 */
@Api(value="MergeAsinController",tags={"ASIN合并信息(手动导入)"})
@RestController
//@ApiIgnore
@AdminAuthToken
@RequestMapping("/pc/data/manualImport/mergeAsin")
public class MergeAsinController {

    @Autowired
    MergeAsinService service;
    ...
    public ResultUtil getPageList(
        ...
        return service.getPageList(pageNum, pageSize, shop, area, asin);
    }
     ...
     */
    @PostMapping("/add")
    public ResultUtil add(@ApiParam(value = "ASIN合并信息", required = true)MergeAsin mergeAsin) {
        return service.add(mergeAsin);
    }
     ...
     */
    @PostMapping("/update")
    public ResultUtil update(@ApiParam(value = "ASIN合并信息", required = true)MergeAsin mergeAsin) {
        return service.update(mergeAsin);
    }
