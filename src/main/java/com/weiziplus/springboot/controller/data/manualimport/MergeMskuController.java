...
import com.weiziplus.springboot.service.data.manualimport.MergeMskuService;
 ...
 * @data 2019/7/19 8:26
 */
@Api(value="MergeMskuController",tags={"MSKU合并信息(手动导入)"})
@RestController
//@ApiIgnore
@AdminAuthToken
@RequestMapping("/pc/data/manualImport/mergeMsku")
public class MergeMskuController {

    @Autowired
    MergeMskuService service;
    ...
    @PostMapping("/add")
    public ResultUtil add(@ApiParam(value = "MSKU合并信息", required = true)MergeMsku mergeMsku) {
        return service.add(mergeMsku);
    }
    ...
    @PostMapping("/update")
    public ResultUtil update(@ApiParam(value = "MSKU合并信息", required = true)MergeMsku mergeMsku) {
        return service.update(mergeMsku);
    }
