 ...
 */
@Service
public class PlacePerspectiveService {
     ...
     * @return
     */
    public ResultUtil getPageList(Integer pageNum, Integer pageSize, String date) {
        PageHelper.startPage(pageNum, pageSize);
        PageUtil pageUtil = PageUtil.pageInfo(sponsoredProductsPlacementReportMapper.getPlacePerspectivePageList(date));
        return ResultUtil.success(pageUtil);
    }

    public ResultUtil getPlacement(String shop,String area) {
        PageUtil pageUtil = PageUtil.pageInfo(sponsoredProductsPlacementReportMapper.getPlacement(shop,area));
        return ResultUtil.success(pageUtil);
    }
     ...
     * @return
     */
    public ResultUtil getPlaceList(Integer pageNum, Integer pageSize,Map map) {
        String dateSign = MapUtils.getString(map,"type");
        List<Map<String,Object>> dataLists = sponsoredProductsPlacementReportMapper.getPlacePerspectiveList(map);
        if(null != dataLists && dataLists.size() > 0){
            ...
            for (Map<String,Object> dataMap: dataLists) {
                placements.add(MapUtils.getString(dataMap,"placement"));
            }
//             对campaign进行去重
            LinkedList<String> sortCampaignLinkedList=new LinkedList<>();
            Iterator<String> it=placements.iterator();
            while (it.hasNext()){
                String str=it.next();
                    ...
                    for(Integer dateWeek:dateSortWeekList){
                        flag:  for (Map<String,Object> temMap: tempCampaignList){
                            ...
                            if(!dateWeek.equals(MapUtils.getInteger(temMap,dateSign))){
                                ...
                                nullMap.put("placement",placement);
            ...
            List<String> sortList = sortedByFields(MapUtils.getString(map,"sortName"),dateSortWeekList.get(0),dataMapLists,"placement", MapUtils.getString(map,"sortRule"),dateSign);
                ...
                flag:for (String nameField : sortList) {
                    ...
                    for (List<Map<String,Object>> list :dataMapLists) {
                        for (Map<String,Object> secondMap: list) {
                            if(nameField.equals(MapUtils.getString(secondMap,"placement"))){
...
//                    }
     ...
     * @return
     */
//    public ResultUtil getList(Integer pageNum, Integer pageSize,Map map) {
//        List dataList = sponsoredProductsPlacementReportMapper.getPlacePerspectiveList(map);
//        //这个initmap需要改成所有数据为0的map，下面方法传的date参数要指定
//        Map initMap = (Map) dataList.get(0);
//        List list = ListUtil.mergeList(dataList,"placement",(String) map.get("date"),initMap);
...
//    }
    ...
    public ResultUtil getCampaignName(String sellerId,String shopArea) {
        return ResultUtil.success(sponsoredProductsPlacementReportMapper.getCampaignName(sellerId,shopArea));
    }
